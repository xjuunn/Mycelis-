...
export class FriendRequestController {
  ...
  async accept(@Param('id') id: string, @Token() tokenInfo: TokenInfo) {
    ...
    if (request?.receiverId == undefined) throw new BadRequestException("错误的请求接收者");
    if (request?.status === "ACCEPTED") throw new BadRequestException("该请求已被同意");
    if (request?.status === "REJECTED") throw new BadRequestException("该请求已被拒绝");
    await this.friendshipService.create({ friendId: request?.receiverId }, request?.senderId);
    await this.friendshipService.create({ friendId: request?.senderId }, request?.receiverId);
