...
export class FriendTagController {
  ...
  @Get('/details/:id')
  listDetails(@Param('id') id: string, @PageInfo() pageInfo: PageRequest, @Token() tokenInfo: TokenInfo) {
    return this.friendTagService.listDetails(+id, pageInfo, tokenInfo.id)
  }

  @ApiOperation({ summary: "创建标签" })
  @Post()
  create(@Body() createFriendTagdto: CreateFriendTagDto, @Token() tokenInfo: TokenInfo) {
    return this.friendTagService.create(createFriendTagdto, tokenInfo.id);
  }

  @ApiOperation({ summary: "修改标签" })
  @Patch(":id")
  update(@Param('id') id:string,@Body() updateFriendTagDto: UpdateFriendTagDto, @Token() tokenInfo: TokenInfo) {
