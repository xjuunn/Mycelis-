...
export class MessageController {
    ...
    @Post('/create')
    create(@Body() createMessageDto: CreateMessageDto, @Token() tokenInfo: TokenInfo) {
        return this.messageService.create(createMessageDto, tokenInfo.id);
    }

    @ApiOperation({ summary: "获取发送的消息列表" })
    @Post("/sentlist")
    findSentMessageList(@Body() searchMessageDto: SearchMessageDto, @Token() tokenInfo: TokenInfo, @PageInfo() pageInfo: PageRequest) {
        return this.messageService.findSentMessageList(searchMessageDto, tokenInfo.id, pageInfo);
    }

    @ApiOperation({ summary: "获取接收的消息列表" })
    @Post("/receivedlist")
    findReceivedMessageList(@Body() searchMessageDto: SearchMessageDto, @Token() tokenInfo: TokenInfo, @PageInfo() pageInfo: PageRequest) {
        return this.messageService.findReceivedMessageList(searchMessageDto, tokenInfo.id, pageInfo)
    }
    ...
    @Patch("/:id")
    update(@Param('id') id: string, @Body() updateMessage: UpdateMessageDto, @Token() tokenInfo: TokenInfo) {
